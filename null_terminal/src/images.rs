...
impl Images {
    ...
    /// Годится ли терминал для картинок прямо в ленте.
    ///
    /// Kitty и iTerm2 рисуют через привязку к ячейкам: картинка едет вместе с
    /// текстом почти бесплатно. Sixel так не умеет — терминал перерисовывает
    /// графику на каждый сдвиг, и при быстрой прокрутке это заметно. Раньше
    /// его из-за этого исключали, но тогда в Windows Terminal — а это главное
    /// окно для большинства — фотографии не показывались вовсе, что хуже
    /// подтормаживания. Кому мешает, выключает настройкой `inline_images`.
    pub fn inline_friendly(&self) -> bool {
        matches!(
