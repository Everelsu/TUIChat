...
const ALPN: &[u8] = b"null-terminal/1";
...
impl Tunnel {
    ...
    /// Сколько прямых адресов у этой стороны уже известно.
    ///
    /// Сразу после запуска их может не быть вовсе — тогда в `addr()` лежит
    /// один релей, и соединение даже с соседней машиной пойдёт через
    /// интернет. Ноль здесь означает «подожди ещё», а не «сети нет».
    pub fn direct_addrs(&self) -> usize {
        self.endpoint
            .addr()
            .addrs
            .iter()
