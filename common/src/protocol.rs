...
pub enum AttachmentKind {
    Image,
    Audio,
    /// Всё остальное: документ, архив, что угодно. Показывать содержимое
    /// такого файла нечем, поэтому клиент рисует имя и размер, а открывает
    /// его человек сам.
    File,
}
...
/// Одна строка в списке комнат сервера: имя и сколько человек внутри.
///
/// Отдаётся отдельным http-запросом `GET /rooms`, а не по WebSocket: список
/// нужен ещё на экране входа, до всякого `Join`, и веб-клиенту его удобно
/// забрать обычным `fetch`. Пустые комнаты в список не попадают — они на
/// сервере не живут, так что показывается ровно то, где сейчас есть люди.
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
