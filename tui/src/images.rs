...
impl Images {
    ...
    ///
    /// Kitty рисует через unicode-плейсхолдеры: картинка привязана к ячейкам
    /// и при прокрутке двигается вместе с текстом почти бесплатно. Sixel же
    /// заставляет терминал перерисовывать графику на каждый сдвиг, и лента
    /// начинает заметно тормозить — поэтому по умолчанию там миниатюр нет.
    pub fn inline_friendly(&self) -> bool {
        matches!(
            self.picker.as_ref().map(Picker::protocol_type),
            Some(ProtocolType::Kitty) | Some(ProtocolType::Iterm2)
        )
    }
